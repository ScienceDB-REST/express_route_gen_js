...
/**
 * Sets the current value of a sequence to the maximum value of the respective
 * primary key column, the sequence normally is used to pupulate.
 *
 * @param {object} sequelizeModel - The model for which to execute the operation
 * @param {string} sequenceName - optional sequence name. Default will be model-name_id_seq
 *
 * @returns {boolean} true if and only if no error ocurrs.
 */
exports.setIdSequenceToCurrentMaximumPrimaryKeyVal = async function(
  sequelizeModel, sequenceName) {
  if (sequenceName === undefined || sequenceName === null)
    sequenceName = `${sequelizeModel.tableName}_id_seq`
  let maxIdRes = await sequelizeModel.sequelize.query(
    `SELECT max("${sequelizeModel.primaryKeyField}") FROM "${sequelizeModel.tableName}"`
  )
  let maxId = (maxIdRes[0][0]['max'] === null || maxIdRes[0][0]['max'] ===
    undefined) ? 0 : maxIdRes[0][0]['max']
  // Update the sequence
  await sequelizeModel.sequelize.query(`SELECT setval('${sequenceName}', ${maxId}, true)`)
  return true
}

/**
 * Checks whether the argument array of objects ('records') contains a first
 * object which has an attribute (key) identical to the argument sequelize
 * model ('model') primary key field.
 *
 * @param {array} records - An array of POJOs possibly generated by the
 * function parseCsv or parseXlsx.
 * @param {object} model - The Sequelize model the argument POJOs ('records')
 * are supposed to be persistes as.
 *
 * @returns {boolean} true if and only if the first POJO in argument 'records'
 * contains an attribute identical to the argument Sequelize 'model'
 * primaryKeyField. Returns false otherwise.
 */
exports.isIdAttributeContained = function(records, model) {
  let idContained = false
  if (Arrays.isArray(records) && records.length > 0) {
    let firstRec = records[0]
    let frKeys = Object.keys(firstRec)
    idContained = frKeys.indexOf(model.primaryKeyField) > -1
  }
  return idContained
}
